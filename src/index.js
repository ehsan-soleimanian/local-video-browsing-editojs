class LocalVideo {
    ...
    constructor({ data, api, readOnly }) {
        this.api = api;
        this.data = data || {};
        this.readOnly = readOnly; // Determine if the block is in read-only mode
        this.wrapper = null;
        this.modal = null;
    }
    ...
    render() {
        ...
        this.wrapper.classList.add("local-file");
        ...
        if (!this.readOnly) {
            ...
            input.readOnly = true; // Make input read-only for clicking
    ...
    fetchVideos() {
        return fetch("/getVideosUploaded") // Replace with your actual endpoint
    ...
    async showModal() {
        ...
        videos.forEach((video) => {
            ...
            videoPreview.setAttribute("controls", "");   // Adds play controls
            videoPreview.setAttribute("preload", "metadata"); // Loads only metadata
            videoPreview.setAttribute("width", "150");   // Set a fixed width
            videoPreview.setAttribute("muted", "");      // Mute video

            // Create a div for video info
    ...
    selectVideo(url) {
        ...
        // Remove any existing video players before adding a new one
